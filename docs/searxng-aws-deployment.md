...
Create `searxng/searxng_settings.yml` with scientific search engines:
...
Create `searxng-deploy/ecs-task-definition.json`:
...
aws ecs register-task-definition \
    --cli-input-json file://searxng-deploy/ecs-task-definition.json \
    --region us-east-1
```
