...
def remove_duplicates(elements: List[dict]) -> List[dict]:
    """
    Remove the duplicates from a list of paragraphs
    """
    result = []
    for elem in elements:
        found = False
        for tmp in result:
            if elem["text"] == tmp["text"]:
                found = True
                break
        if not found:
            result.append(elem)
...
class ContextRetriever:
    ...
    def __init__(
        ...
        Args:
            ...
            service_type: str
                The version of the context retriever (chromadb, langchain, ...)
        """
    ...
    def query(
        ...
        if self.service_type == "chromadb":
            if self.db_remote:
                ...
                if granularity == "paragraph":
                    if relevance == True:
                        ...
                        passages = []
                        for i, doc in enumerate(docs):
                            title = topic if topic is not None else get_title(doc)
                            passages.extend([dict(title=title, text=par) for par in split_paragraphs(text=doc)])
                elif granularity == "document":
                    passages = [dict(title=get_title(doc), text=make_uniform(doc)) for doc in docs]
                else:
            ...
            if self.debug:
                print(f"Retrieving {top_k} relevant documents for query: {text}")
            ...
            wiki_retriever = WikipediaRetriever(lang="en", top_k_results=top_k)

            passages = []
            if granularity == "paragraph":
                paragraphs = []
                rel_docs = []
                # Get the most relevant doc to the topic
                # if topic is not None: 
                #     docs = wiki_retriever.invoke(topic)
                #     if len(docs) > 0:
                #         rel_docs.append(docs[0])
                # Get most relevant docs to the query
                rel_docs.extend(wiki_retriever.invoke(text))
                for doc in rel_docs:
                    title = doc.metadata["title"]
                    summary = doc.metadata["summary"]
                    # doc_content = summary + " " + doc.page_content
                    doc_content = doc.page_content
                    paragraphs.extend([dict(title=title, text=para) for para in get_paragraphs(doc_content)])

                # Remove duplicates from `paragraphs`
                ...
                for doc in rel_docs:
                    ...
                    passages.append(dict(title=title, text=doc_content))
            
            # Extract the top_k passages
            n = len(passages) if top_k is None else min(top_k, len(passages))
            for i in range(n):
                results.append(passages[i]) # a passage is a dict with title and text as keys
        elif self.service_type == "serper":
            ...
            headers = {
                'X-API-KEY': 'ef2168c2c5337119397c4d67234151d80e6ebe1e',
                'Content-Type': 'application/json'
            }

            response = requests.request("POST", url, headers=headers, data=payload)
            results.append({"title": text, "text": response.text})
            print(response.text)
