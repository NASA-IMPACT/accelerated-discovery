...
class PlanStep(BaseModel):
    ...
    inputs: Dict[str, Any] = Field(
        default_factory=dict, description="Inputs required for this step"
    )
    outputs: Dict[str, Any] = Field(
        default_factory=dict, description="Outputs produced by this step"
    )
    dependencies: List[str] = Field(
        default_factory=list, description="Step IDs that must complete before this step"
    )
    status: AgentStatus = Field(
        default=AgentStatus.PENDING, description="Current status of this step"
    )
    priority: int = Field(
    ...
    human_review_required: bool = Field(
        default=False, description="Whether this step requires human review"
    )
...
class ResearchPlan(BaseModel):
    ...
    steps: Dict[str, PlanStep] = Field(
        default_factory=dict, description="Steps in the plan"
    )
    ...
    blocked_steps: List[str] = Field(
        default_factory=list, description="List of blocked step IDs"
    )
    human_feedback: List[Dict[str, Any]] = Field(
        default_factory=list, description="Human feedback and instructions"
    )
...
class HumanInteraction(BaseModel):
    ...
    context: Dict[str, Any] = Field(
        default_factory=dict, description="Context information for the human"
    )
    ...
    timeout_minutes: Optional[int] = Field(
        None, description="Timeout for this interaction"
    )
...
class LiteratureSearchState(BaseModel):
    ...
    filtered_results: List[SearchResultItem] = Field(
        default_factory=list, description="Relevancy-filtered results"
    )
    extraction_results: List[Dict[str, Any]] = Field(
        default_factory=list, description="Extracted information"
    )
    search_iteration: int = Field(default=0, description="Current search iteration")
    relevancy_scores: Dict[str, float] = Field(
        default_factory=dict, description="Relevancy scores by URL"
    )
    search_strategy: str = Field(
        default="initial", description="Current search strategy"
    )
...
class ExtractionState(BaseModel):
    ...
    extraction_schema: Optional[Dict[str, Any]] = Field(
        None, description="Schema for extraction"
    )
    raw_content: List[str] = Field(
        default_factory=list, description="Raw content to extract from"
    )
    extracted_data: List[Dict[str, Any]] = Field(
        default_factory=list, description="Extracted structured data"
    )
...
class PlannerState(BaseModel):
    ...
    current_plan: Optional[ResearchPlan] = Field(
        None, description="Current active research plan"
    )
    plan_history: List[ResearchPlan] = Field(
        default_factory=list, description="History of plans"
    )
    available_agents: List[str] = Field(
        default_factory=list, description="Available agent types"
    )
    agent_capabilities: Dict[str, List[str]] = Field(
        default_factory=dict, description="Capabilities of each agent type"
    )
    planning_context: Dict[str, Any] = Field(
        default_factory=dict, description="Context for planning decisions"
    )
...
class GlobalWorkflowState(NodeState):
    ...
    workflow_status: Literal[
        "initializing", "planning", "executing", "waiting_human", "completed", "failed"
    ...
    pending_human_interaction: Optional[HumanInteraction] = Field(
        None, description="Current pending human interaction"
    )
    ...
    def complete_human_interaction(
        self, interaction_id: str, response: Dict[str, Any]
    ) -> bool:
