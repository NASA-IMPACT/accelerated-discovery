...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if state and state.values: # restore the state if the thread is already present.
        ...
        [await websocket.send_text(f"{message.message_type}:{message.message}") for message in messages]
    while True:
        ...
        await stream_graph_updates(user_message.message,graph,websocket,config["configurable"])
...
async def get_past_messages(config: dict, graph:CompiledStateGraph) -> list[Message]:
    ...
    if state.values and len(state.values["messages"])>0:
        for message in state.values["messages"]:
            message_obj = Message(thread_id=config["configurable"]["thread_id"], message_type= type(message).__name__,message=message.content)
