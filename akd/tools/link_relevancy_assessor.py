...
class LinkRelevancyAssessor(BaseTool):
    ...
    def _calculate_relevancy_score(self, assessment: MultiRubricRelevancyOutputSchema) -> float:
        """Calculate a numeric relevancy score from multi-rubric assessment."""
        # Map enum values to scores
        rubric_scores = []
        
        # Topic alignment (weight: 0.25)
        topic_score = 1.0 if assessment.topic_alignment.value == "aligned" else 0.0
        rubric_scores.append(topic_score * 0.25)
        
        # Content depth (weight: 0.15)
        depth_score = 1.0 if assessment.content_depth.value == "comprehensive" else 0.0
        rubric_scores.append(depth_score * 0.15)
        
        # Evidence quality (weight: 0.25)
        evidence_score = 1.0 if assessment.evidence_quality.value == "high_quality_evidence" else 0.0
        rubric_scores.append(evidence_score * 0.25)
        
        # Methodological relevance (weight: 0.15)
        method_score = 1.0 if assessment.methodological_relevance.value == "methodologically_sound" else 0.0
        rubric_scores.append(method_score * 0.15)
        
        # Recency relevance (weight: 0.10)
        recency_score = 1.0 if assessment.recency_relevance.value == "current" else 0.0
        rubric_scores.append(recency_score * 0.10)
        
        # Scope relevance (weight: 0.10)
        scope_score = 1.0 if assessment.scope_relevance.value == "in_scope" else 0.0
        rubric_scores.append(scope_score * 0.10)
        
        total_score = sum(rubric_scores)
        
        if self.debug:
            logger.debug(f"🧮 RELEVANCY SCORE BREAKDOWN:")
            logger.debug(f"  📍 Topic alignment: {assessment.topic_alignment.value} -> {topic_score:.2f} * 0.25 = {topic_score * 0.25:.3f}")
            logger.debug(f"  📊 Content depth: {assessment.content_depth.value} -> {depth_score:.2f} * 0.15 = {depth_score * 0.15:.3f}")  
            logger.debug(f"  🏆 Evidence quality: {assessment.evidence_quality.value} -> {evidence_score:.2f} * 0.25 = {evidence_score * 0.25:.3f}")
            logger.debug(f"  🔬 Methodological: {assessment.methodological_relevance.value} -> {method_score:.2f} * 0.15 = {method_score * 0.15:.3f}")
            logger.debug(f"  ⏰ Recency: {assessment.recency_relevance.value} -> {recency_score:.2f} * 0.10 = {recency_score * 0.10:.3f}")
            logger.debug(f"  🎯 Scope: {assessment.scope_relevance.value} -> {scope_score:.2f} * 0.10 = {scope_score * 0.10:.3f}")
    ...
    async def _assess_single_result(
        ...
        result.should_fetch_full_content = relevancy_score >= self.full_content_threshold
    ...
    async def _assess_query_alignment(
        ...
        for i in range(0, len(results), self.assessment_batch_size):
            ...
            if reformulated_query and reformulated_query != original_query:
                reformulated_tasks = [
                    self._assess_single_result(result, reformulated_query, domain_context)
                ...
                for orig_result, reform_result in zip(batch_results, reformulated_results):
                    if reform_result.relevancy_score > orig_result.relevancy_score:
                        ...
                        orig_result.relevancy_assessment = reform_result.relevancy_assessment
                        orig_result.should_fetch_full_content = reform_result.should_fetch_full_content
    ...
    def _create_assessment_summary(self, results: List[SearchResultItem]) -> Dict[str, Any]:
        ...
        scores = [r.relevancy_score for r in assessed_results if r.relevancy_score is not None]
        ...
        return {
            ...
            "high_relevancy_count": len([r for r in results if r.should_fetch_full_content]),
            "filtered_count": len([r for r in assessed_results if r.relevancy_score >= self.min_relevancy_score]),
    ...
    async def _arun(
        ...
        **kwargs,
        ...
        filtered_results = [
            result for result in assessed_results
            if result.relevancy_score is not None and result.relevancy_score >= self.min_relevancy_score
        ]
        
        # Identify high-relevancy results for full content fetching
        high_relevancy_results = [
            result for result in assessed_results
            if result.should_fetch_full_content
        ]
