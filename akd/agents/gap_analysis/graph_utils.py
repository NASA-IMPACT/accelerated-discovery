        ...
        if isinstance(paper, ParsedPaper):
            ...
            for i, section in enumerate(sections, start=1):
                ...
                if section_title not in section_to_key:
                    print(
                        f"Section {section_title} missing for {paper_data.get('title')}",
                    )
                    continue
...
async def select_subsections(
    connected_subsection_nodes: List,
    query: str,
    llm,
) -> List[str]:
    """
    ...
    if len(subsection_titles) != 0:
        ...
        selected_subsection_nodes = [
            title_to_node_id_map[subsection_title]
            for subsection_title in selected_subsection_titles
        ]
...
async def retrieve_relevant_sections(
    ...
    selected_subsection_nodes = await select_subsections(
        connected_subsection_nodes,
        query,
        llm,
    )
...
async def generate_final_answer(
    ...
    all_answers = await asyncio.gather(
        *[
            local_answer_chain.abatch(answer_prompt_data)
