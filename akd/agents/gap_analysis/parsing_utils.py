...
def parse_html(html_content: str) -> List[Dict[str, str]]:
    ...
    for section in sections:
        ...
        if section_title not in parsed_content:
            ...
            raise KeyError(f"section_title {section_title} has been parsed or is being duplicated")
        ...
        if len(section_title_list) > 1:
            for sub_title in section_title_list[1:]:
                ...
                if sub_content is not None:
                    subsections_data.append(SubSection(title=sub_title, content=sub_content))
        if subsections_data:
            sections.append(Section(title=main_section_title, content=section_content, subsections=subsections_data))
        else:
            if section_content is not None:
                sections.append(Section(title=main_section_title, content=section_content, subsections=None))
            else: 
                sections.append(Section(title=main_section_title, content=None, subsections=None))
...
async def group_section_titles(parsed_output: list[dict], llm):
    ...
    model_output = await group_sections_chain.ainvoke({"input_sections": section_titles})
