...
class GapOutputSchema(OutputSchema):
    ...
    attributed_source_answers: Dict = Field(..., description="Answers for each node selected from the graph.")
...
class GapAgent(BaseAgent):
    ...
    async def get_response_async(
        ...
        **kwargs,
        ...
        if self.debug:
            logger.debug(f"Selected nodes from {len(selected_nodes)} papers. Now generating answer")
        output, attributed_source_answers = await generate_final_answer(G, query=gap, all_selected_nodes=selected_nodes, llm=self.llm)
        return GapOutputSchema(output=output, attributed_source_answers=attributed_source_answers, G=G)
