...
from akd.agents.search.aspect_search.interview_utils import (
    generate_answer,
    generate_question,
    route_messages,
    survey_subjects,
)
from akd.agents.search.aspect_search.structures import (
    InterviewState,
    Perspectives,
    update_references,
...
class AspectSearchOutputSchema(OutputSchema):
    ...
    search_results: Union[List[SearchResultItem], List[List[SearchResultItem]]] = Field(
        None,
        description="List of search results returned by the search engine after the interviews.",
    )
    references: Union[Dict[str, str], List[Dict[str, str]]] = Field(
        None,
        description="List of references.",
    )
    perspectives: Union[Perspectives, List[Perspectives]] = Field(
        None,
...
class AspectSearchConfig(BaseAgentConfig):
    ...
    num_editors: Optional[int] = Field(
        default=3,
...
class AspectSearchAgent(BaseAgent):
    ...
    async def get_perspectives(self, topic: str) -> List[Perspectives]:
        ...
        """
        perspectives = await survey_subjects.ainvoke(
            topic,
            llm=self.llm,
    ...
    async def _conduct_interviews(self, topic: str) -> List[Dict]:
        ...
        if self.debug:
            logger.debug("🤖: Here are your editors!")
            for editor in editors:
                logger.debug(
        ...
        references = {}
        for interview in interview_results:
            search_results = update_search_results(
                search_results,
