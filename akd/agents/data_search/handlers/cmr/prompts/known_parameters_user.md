...
4. **Universality**: If spatial, temporal, or resolution requirements are essential to the question then they should universally apply to all query approaches generated. Do not apply spatial/temporal/resolution values piecemeal unless there is a very good reason to do so.
5. **Multi-Temporal Comparisons**: For queries comparing multiple non-contiguous time periods, create SEPARATE query approaches (one per time period). NEVER combine multiple temporal ranges with semicolons. Each temporal parameter must be exactly "start,end" format.
