...
from akd.agents.aspect_search import (
    AspectSearchAgent,
    AspectSearchConfig,
    AspectSearchInputSchema,
    AspectSearchOutputSchema,
)
from akd.agents.aspect_search.structures import Perspectives, Editor
...
def agent():
    ...
    config = AspectSearchConfig(model_name='gpt-4o-mini',
                                api_key=openai_key,                    
                                debug=True,
                                max_turns=2,
                                num_editors=1,
...
def dummy_perspectives():
    return Perspectives(editors=[
        Editor(affiliation='University Researcher', 
               name='Dr. Alice Chen', 
...
def dummy_interview_result(dummy_perspectives):
    return ([SearchResultItem(url="http://example.com", title="test", query="test", content="content")],
...
def dummy_interview():
    return [{
            "messages": [MagicMock(content="test message", name="Subject_Matter_Expert")],
            "search_results": [SearchResultItem(url="http://example.com", title="test", query="test", content="content")],
...
async def test_get_perspectives(mock_survey_subjects, agent, dummy_perspectives, dummy_topic):
...
@pytest.mark.asyncio
@patch("akd.agents.aspect_search.aspect_search.survey_subjects")
async def test_conduct_interviews(mock_survey_subjects, 
                                  agent, 
                                  dummy_topic, 
                                  dummy_perspectives,
                                  dummy_interview):
    ...
    search_results, references, perspectives = await agent._conduct_interviews(dummy_topic)
...
@pytest.mark.asyncio
async def test_get_response_async_single_topic(agent, dummy_topic, dummy_interview_result):
...
@pytest.mark.asyncio
async def test_get_response_async_multiple_topics(agent, dummy_interview_result, dummy_topic):
...
async def test_arun(agent, dummy_topic):
    ...
    params = AspectSearchInputSchema(topic=dummy_topic)
    agent.get_response_async = AsyncMock(return_value=AspectSearchOutputSchema(search_results=[], references={}, perspectives=Perspectives(editors=[])))
